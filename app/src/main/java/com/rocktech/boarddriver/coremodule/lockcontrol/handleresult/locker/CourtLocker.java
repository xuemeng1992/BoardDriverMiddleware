...
public class CourtLocker implements ILocker, OnDataReceiverListener {
    ...
    private static volatile CourtLocker locker;

    public static CourtLocker getInstance(Context context, int type) {
        if (locker == null) {
            synchronized (CourtLocker.class) {
                if (locker == null) {
                    locker = new CourtLocker(context, type);
                }
            }
        }
        return locker;
    ...
    private CourtLocker(Context context, int type) {
        ...
        switch (handle) {
            ...
            case Constant.STATE_QUERY_SINGLE_LOCK:
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    ...
                    querySingleIntent.putExtra("isopened", HandleFactory.produceHandle(type).stateQuerySingleLock(data, HandleFactory.getCurrentBoxId(), context));
            ...
            case Constant.STATE_OPEN_SINGLE_LOCK:
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    ...
                    openSingleIntent.putExtra("isopened", HandleFactory.produceHandle(type).stateOpenSingleLock(data, HandleFactory.getCurrentBoxId(), context));
