...
import com.rocktech.boarddriver.coremodule.lockcontrol.handleresult.customized.Raoping;
...
public class RaopingLocker implements ILocker, OnDataReceiverListener {
    ...
    private static volatile RaopingLocker locker;

    public static RaopingLocker getInstance(Context context, int type) {
        if (locker == null) {
            synchronized (RaopingLocker.class) {
                if (locker == null) {
                    locker = new RaopingLocker(context, type);
                }
            }
        }
        return locker;
    ...
    private RaopingLocker(Context context, int type) {
        lockerControl = new LockerControl(ConfigureTools.getCustomerConfigBean(context, Constant.CustomerTYPE.QIAOMU).getCom(), Constant.LOCKER_BAUDRATE);
        ...
        switch (handle) {
            ...
            case Constant.STATE_QUERY_SINGLE_LOCK:
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    ...
                    querySingleIntent.putExtra("isopened", Raoping.getRaopingOpenRes(type, HandleFactory.getCurrentBoxId(), data, context));
                    context.sendBroadcast(querySingleIntent);
                    commonDataObservable.setData(new CommonDataBean(Raoping.getRaopingOpenRes(type, HandleFactory.getCurrentBoxId(), data, context)));
            ...
            case Constant.STATE_OPEN_SINGLE_LOCK:
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    ...
                    openSingleIntent.putExtra("isopened", Raoping.getRaopingOpenRes(type, HandleFactory.getCurrentBoxId(), data, context));
            ...
            case Constant.STATE_QUERY_ALL_LOCK_FOR_COURT:
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    List<Boolean> openList = HandleFactory.produceHandle(type).checkAllLockState(data, HandleFactory.getCurrentBoxId(), HandleFactory.getCurrentAssetCode(), context);
                    boxDataObservable.addOpenedList(openList);
            ...
            case Constant.STATE_OPEN_MUTI_LOCK:
                ...
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    boxDataObservable.addHasOpened(true);
                    boxDataObservable.setIndex(boxDataObservable.getIndex() + 1);
                }
                break;
            ...
            case Constant.STATE_QUERY_OPENED_LOCK:
                ...
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    boxDataObservable.addHasOpened(Raoping.getRaopingOpenRes(type, HandleFactory.getCurrentBoxId(), data, context));
                    boxDataObservable.setIndex(boxDataObservable.getIndex() + 1);
                }
                break;
    ...
    public void writeCode(final List<AssetCodeBean> list) {
        ...
        AssetCodeBean assetCodeBean = list.get(0);
        List<AssetCodeBean> newList = new ArrayList<>();
        if (assetCodeBean.getBoxId().contains("C")) {
            newList.add(assetCodeBean);
            AssetCodeBean d = new AssetCodeBean(assetCodeBean.getAssetCode(), "C**", assetCodeBean.getOriginalAssetCode(), assetCodeBean.getProductModel(), "D**");
            newList.add(d);
        } else if (assetCodeBean.getBoxId().contains("D")) {
            AssetCodeBean e = new AssetCodeBean(assetCodeBean.getAssetCode(), "D**", assetCodeBean.getOriginalAssetCode(), assetCodeBean.getProductModel(), "E**");
            newList.add(e);
            AssetCodeBean f = new AssetCodeBean(assetCodeBean.getAssetCode(), "D**", assetCodeBean.getOriginalAssetCode(), assetCodeBean.getProductModel(), "F**");
            newList.add(f);
        } else {
            newList.add(assetCodeBean);
        }
        lockerControl.addData(CMDFactory.produceCMD(type).writeAssetCode(newList.get(0)), false, Constant.FLAG_WRITE_ASSET_CODE);
        DataObserver dataObserver = new DataObserver();
        dataObserver.setiWriteAssetCode(new DataObserver.IWriteAssetCode() {
            ...
            public void writeAssetCode(WriteAssetCodeObservable writeAssetCodeObservable) {
                if (writeAssetCodeObservable.getIndex() < newList.size()) {
                    lockerControl.addData(CMDFactory.produceCMD(type).writeAssetCode(newList.get(writeAssetCodeObservable.getIndex())), false, Constant.FLAG_WRITE_ASSET_CODE);
                } else {
    ...
    public synchronized boolean openLockSyn(String str, int boxIndex, AssetCodeDao assetCodeDao) {
        ...
        String bName;
        if (boxIndex != -1) {
            String boxId = Raoping.calculateBoardEx(assetCodeDao.getAssetCode(), boxIndex);
            int newCellIndex = Raoping.getNewCellIndex(assetCodeDao.getBoxId().substring(0, 1), boxIndex);
            String bIndex = String.format("%02d", newCellIndex);
            bName = boxId + bIndex;
        } else {
            bName = str;
        }
    ...
    public synchronized CommonDataBean openLocksSyn(final String[] batchboxid) {
        Log.e("openLocksSyn", Arrays.asList(batchboxid).toString());
        final CountDownLatch countDownLatch = new CountDownLatch(1);
        boxDataObservable.initData(batchboxid[0]);
        HandleFactory.setCurrentBoxId(batchboxid[0]);
        lockerControl.addData(CMDFactory.produceCMD(type).openLock(batchboxid[0]), false, Constant.STATE_OPEN_MUTI_LOCK);
    ...
    public synchronized boolean queryLockSyn(String str, int boxIndex, AssetCodeDao assetCodeDao) {
        ...
        if (boxIndex != -1) {
            ...
            int newCellIndex = Raoping.getNewCellIndex(assetCodeDao.getBoxId().substring(0, 1), boxIndex);
            String bIndex = String.format("%02d", newCellIndex);
            queryBName = boxId + bIndex;
        } else {
            queryBName = str;
        }
        HandleFactory.setCurrentBoxId(queryBName);
        lockerControl.addData(CMDFactory.produceCMD(type).queryLock(queryBName), false, Constant.STATE_QUERY_SINGLE_LOCK);
        DataObserver dataObserver = new DataObserver();
        dataObserver.setiCommonDataObservable(new DataObserver.ICommonDataObservable() {
            @Override
            public void commonDataObservable(CommonDataObservable commonDataObservable) {
    ...
    public synchronized List<Boolean> queryAllLockSyn(String cabinet, String assetCode) {
        String[] newBatchBoxId;
        if (!"C".equals(cabinet) && !"D".equals(cabinet)) {
            newBatchBoxId = new String[]{cabinet + "01"};
        } else {
            if ("C".equals(cabinet)) {
                newBatchBoxId = new String[]{"C01", "D01"};
            } else {
                newBatchBoxId = new String[]{"E01", "F01"};
            }
        }
        final CountDownLatch countDownLatch = new CountDownLatch(1);
        HandleFactory.setCurrentAssetCode(assetCode);
        HandleFactory.setCurrentBoxId(newBatchBoxId[0]);
        boxDataObservable.initData(newBatchBoxId[0]);
        lockerControl.addData(CMDFactory.produceCMD(type).queryLock(newBatchBoxId[0]), false, Constant.STATE_QUERY_ALL_LOCK_FOR_COURT);
        DataObserver dataObserver = new DataObserver();
        dataObserver.setiUpdataBox(new DataObserver.IUpdataBox() {
            @Override
            public void updataBox(BoxDataObservable boxDataObservable) {
                if (boxDataObservable.getIndex() < newBatchBoxId.length) {
                    HandleFactory.setCurrentBoxId(newBatchBoxId[boxDataObservable.getIndex()]);
                    HandleFactory.setCurrentAssetCode(assetCode);
                    boxDataObservable.addbatchBoxId(newBatchBoxId[boxDataObservable.getIndex()]);
                    lockerControl.addData(CMDFactory.produceCMD(type).queryLock(newBatchBoxId[boxDataObservable.getIndex()]), false, Constant.STATE_QUERY_ALL_LOCK_FOR_COURT);
                } else {
                    countDownLatch.countDown();
                }
            }
        ...
        CommonDataBean bean = new CommonDataBean(boxDataObservable.getHasOpeneds());
        if (bean.getO1() != null) {
            boxDataObservable.deleteObservers();
    ...
    public synchronized List<Boolean> writeCodeSyn(final List<AssetCodeBean> list) {
        ...
        Log.e("writeCodeSyn", newList.toString());
        final CountDownLatch countDownLatch = new CountDownLatch(1);
        writeAssetCodeObservable.initData();
        lockerControl.addData(CMDFactory.produceCMD(type).writeAssetCode(newList.get(0)), false, Constant.FLAG_WRITE_ASSET_CODE);
        DataObserver dataObserver = new DataObserver();
        ...
        Log.e("writeCodeSyn:Res:", writeAssetCodeObservable.toString());
