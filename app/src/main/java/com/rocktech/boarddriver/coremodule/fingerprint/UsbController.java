...
public class UsbController {
     ...
     */
    public UsbController(Context parentActivity, IUsbConnState connectionHandler, int vid,
                         int pid) {
...
//		init();
    ...
    public void init() {
        enumerate(new IPermissionListener() {
            ...
            public void onPermissionDenied(UsbDevice d) {
                UsbManager usbman =
                        (UsbManager) mApplicationContext.getSystemService(Context.USB_SERVICE);
                PendingIntent pi = PendingIntent.getBroadcast(mApplicationContext, 0,
    ...
    private void enumerate(IPermissionListener listener) {
        ...
        while (deviter.hasNext()) {
            ...
            LoggerUtils.Log().e("Found device: " + String.format("%04X:%04X", d.getVendorId(),
                    d.getProductId()));
//			DebugManage.WriteLog2("Found device: " + String.format("%04X:%04X", d.getVendorId(), d
//			.getProductId()));

            if (d.getVendorId() == VID && d.getProductId() == PID) {
                bFound = true;
                if (!mUsbManager.hasPermission(d)) {
                    LoggerUtils.Log().e("enumerate, hasPermission return false");
                    listener.onPermissionDenied(d);
                } else {
                    LoggerUtils.Log().e("enumerate, GetConnInerface start");
                    //startHandler(d);
                    GetConnInerface(d);
                    //TestComm(d);
                    return;
        ...
        if (bFound == false) {
            LoggerUtils.Log().e("no more devices found");
...
//        m_usbDevice = dev;
        ...
        //Toast.makeText(mApplicationContext, "GetConnInerface OK, Out Max Size="+m_nEPOutSize+"
        // In Max Size=" + m_nEPInSize, Toast.LENGTH_SHORT).show();
...
//        DebugManage.WriteLog2("device connected");
    ...
    public boolean UsbSCSIWrite(byte[] pCDB, int nCDBLen, byte[] pData, int nDataLen,
                                int nTimeOut) {
        byte[] w_abyTmp = new byte[31];
        byte[] w_abyCSW = new byte[13];
        boolean w_bRet;
    ...
    public boolean UsbSCSIRead(byte[] pCDB, int nCDBLen, byte[] pData, int nDataLen, int nTimeOut) {
        ...
        //Toast.makeText(mApplicationContext, "UsbSCSIRead, UsbBulkReceive Time : " + w_nTime ,
    ...
    private boolean UsbBulkReceive(byte[] pBuf, int nLen, int nTimeOut) {
        ...
        //Toast.makeText(mApplicationContext, "UsbBulkReceive, Buf Len = " + pBuf.length, Toast
        // .LENGTH_SHORT).show();
