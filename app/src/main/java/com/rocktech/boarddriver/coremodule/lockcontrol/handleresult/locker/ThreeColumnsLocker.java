...
import com.rocktech.boarddriver.coremodule.lockcontrol.handleresult.datatransfer.LampObservable;
...
public class ThreeColumnsLocker implements ILocker, OnDataReceiverListener {
    ...
    private LampObservable lampObservable;

    @SuppressLint("StaticFieldLeak")
    private static volatile ThreeColumnsLocker commonLocker;

    public static ThreeColumnsLocker getInstance(Context context, int type) {
        if (commonLocker == null) {
            synchronized (ThreeColumnsLocker.class) {
                if (commonLocker == null) {
                    commonLocker = new ThreeColumnsLocker(context, type);
                }
            }
        }
        return commonLocker;
    ...
    private ThreeColumnsLocker(Context context, int type) {
        lockerControl = new LockerControl(ConfigureTools.getCustomerConfigBean(context, Constant.CustomerTYPE.PANJI).getCom(), Constant.LOCKER_BAUDRATE);
        ...
        this.lampObservable = new LampObservable();
    }
    ...
    private void toHandle(byte[] data, int checkResultTYPE, int handle) {
        ...
        switch (handle) {
            ...
            case Constant.STATE_QUERY_SINGLE_LOCK:
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    Intent querySingleIntent = new Intent("android.intent.action.hal.iocontroller.querydata");
                    querySingleIntent.putExtra("boxid", HandleFactory.getCurrentBoxId());
                    querySingleIntent.putExtra("isopened", HandleFactory.produceHandle(type).stateQuerySingleLock(data, ThreeColumns.getRealBoxName(HandleFactory.getCurrentBoxId()), context));
                    context.sendBroadcast(querySingleIntent);
            ...
            case Constant.STATE_QUERY_DOOR_MAGNET:
                ...
                break;
            case Constant.CLOSELAMP:
            case Constant.OPENLAMP:
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    lampObservable.setIndex(lampObservable.getIndex() + 1);
                } else if (checkResultTYPE == Constant.CheckResultTYPE.OVERTIME) {
                    lampObservable.setIndex(-99);
                }
                break;
            default:
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
    ...
    @Override
    public synchronized void openLock(final String[] batchboxid) {
        final CountDownLatch countDownLatch = new CountDownLatch(1);
        boxDataObservable.initData(batchboxid[0]);
        HandleFactory.setCurrentBoxId(ThreeColumns.getRealBoxName(batchboxid[0]));
        lockerControl.addData(CMDFactory.produceCMD(type).openLock(ThreeColumns.getRealBoxName(batchboxid[0])), false, Constant.STATE_OPEN_MUTI_LOCK);
        DataObserver dataObserver = new DataObserver();
        dataObserver.setiUpdataBox(new DataObserver.IUpdataBox() {
            ...
            public void updataBox(BoxDataObservable boxDataObservable) {
                if (boxDataObservable.getIndex() < batchboxid.length) {
                    ...
                    HandleFactory.setCurrentBoxId(ThreeColumns.getRealBoxName(batchboxid[boxDataObservable.getIndex()]));
                    lockerControl.addData(CMDFactory.produceCMD(type).openLock(ThreeColumns.getRealBoxName(batchboxid[boxDataObservable.getIndex()])), false, Constant.STATE_OPEN_MUTI_LOCK);
                } else {
    ...
    public synchronized void queryLock(String str) {
        lockerControl.addData(CMDFactory.produceCMD(type).queryLock(ThreeColumns.getRealBoxName(str)), false, Constant.STATE_QUERY_SINGLE_LOCK);
        HandleFactory.setCurrentBoxId(str);
    }

    @Override
    public synchronized void queryLock(final String[] batchboxid) {
        final CountDownLatch countDownLatch = new CountDownLatch(1);
        boxDataObservable.initData(batchboxid[0]);
        HandleFactory.setCurrentBoxId(ThreeColumns.getRealBoxName(batchboxid[0]));
        lockerControl.addData(CMDFactory.produceCMD(type).queryLock(ThreeColumns.getRealBoxName(batchboxid[0])), false, Constant.STATE_QUERY_MUTI_LOCK);
        DataObserver dataObserver = new DataObserver();
        dataObserver.setiUpdataBox(new DataObserver.IUpdataBox() {
            @Override
            public void updataBox(BoxDataObservable boxDataObservable) {
                if (boxDataObservable.getIndex() < batchboxid.length) {
                    boxDataObservable.addbatchBoxId(batchboxid[boxDataObservable.getIndex()]);
                    HandleFactory.setCurrentBoxId(ThreeColumns.getRealBoxName(batchboxid[boxDataObservable.getIndex()]));
                    lockerControl.addData(CMDFactory.produceCMD(type).queryLock(ThreeColumns.getRealBoxName(batchboxid[boxDataObservable.getIndex()])), false, Constant.STATE_QUERY_MUTI_LOCK);
                } else {
    ...
    @Override
    public synchronized void queryAll(String boxids) {
        final CountDownLatch countDownLatch = new CountDownLatch(1);
        ...
        boxDataObservable.initData(batchboxid[0]);
        HandleFactory.setCurrentBoxId(ThreeColumns.getRealBoxName(batchboxid[0]));
        lockerControl.addData(CMDFactory.produceCMD(type).queryLock(ThreeColumns.getRealBoxName(batchboxid[0])), false, Constant.STATE_QUERY_OPENED_LOCK);
        DataObserver dataObserver = new DataObserver();
        dataObserver.setiUpdataBox(new DataObserver.IUpdataBox() {
            @Override
            public void updataBox(BoxDataObservable boxDataObservable) {
                if (boxDataObservable.getIndex() < batchboxid.length) {
                    boxDataObservable.addbatchBoxId(batchboxid[boxDataObservable.getIndex()]);
                    HandleFactory.setCurrentBoxId(ThreeColumns.getRealBoxName(batchboxid[boxDataObservable.getIndex()]));
                    lockerControl.addData(CMDFactory.produceCMD(type).queryLock(ThreeColumns.getRealBoxName(batchboxid[boxDataObservable.getIndex()])), false, Constant.STATE_QUERY_OPENED_LOCK);
                } else {
    ...
    public synchronized void controlLamp(boolean isOpen) {
        ...
        lampObservable.initIndex(1);
        lockerControl.addData(CMDFactory.produceCMD(type).controlLamp(1, isOpen), false, currentHandle);
        DataObserver dataObserver = new DataObserver();
        dataObserver.setiLampObservable(new DataObserver.ILampObservable() {
            @Override
            public void lampObservable(LampObservable lampObservable) {
                if (lampObservable.getIndex() != -99) {
                    lockerControl.addData(CMDFactory.produceCMD(type).controlLamp(lampObservable.getIndex(), isOpen), false, currentHandle);
                } else {
                    countDownLatch.countDown();
                }
            }
        });
        lampObservable.addObserver(dataObserver);
        try {
            countDownLatch.await();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        lampObservable.deleteObservers();
    }

    @Override
    public synchronized void writeCode(final List<AssetCodeBean> list) {
    ...
    @Override
    public synchronized void readCodes() {
        final CountDownLatch countDownLatch = new CountDownLatch(1);
    ...
    private void sendCodes(AssetCodeObservable assetCodeObservable) {
        if (DaoTools.getAssetCodes().size() == 0) {
            saveAssetCode(assetCodeObservable);
        }
        ArrayList<String> code1 = new ArrayList<>();//资产编码
        ArrayList<String> code2 = new ArrayList<>();//板编号
        List<AssetCodeBean> newList = ThreeColumns.rmRepeatAssetCode(assetCodeObservable.getCodeBeans());
        for (int i = 0; i < newList.size(); i++) {
            code1.add(newList.get(i).getAssetCode());
            code2.add(newList.get(i).getBoxId());
        }
    ...
    @Override
    public synchronized boolean saveCodes() {
        final CountDownLatch countDownLatch = new CountDownLatch(1);
        assetCodeObservable.initData();
        lockerControl.addData(CMDFactory.produceCMD(type).readCode(0), false, Constant.STATE_HITESTER_READ_ASSETCODE);
        DataObserver dataObserver = new DataObserver();
    ...
    private void saveAssetCode(AssetCodeObservable assetCodeObservable) {
        ...
        for (int i = 0; i < assetCodeObservable.getCodeBeans().size(); i++) {
            ...
            int assetCodeLength = CustomizedFactory.getCustomized(ConfigureTools.getLockerTypeBean(context).getId()).getAssetCodeLength();
            assetCodeDao.setAssetCode(assetCodeObservable.getCodeBeans().get(i).getAssetCode().substring(3, assetCodeLength));
            assetCodeDao.setBoxId(assetCodeObservable.getCodeBeans().get(i).getBoxId());
            assetCodeDao.setAssetCodeTitle(assetCodeObservable.getCodeBeans().get(i).getAssetCode().substring(0, 3));
