...
import com.rocktech.boarddriver.coremodule.lockcontrol.handleresult.customized.Procuratorate;
...
public class ProcuratorateLocker implements ILocker, OnDataReceiverListener {
    ...
    private static volatile ProcuratorateLocker locker;

    public static ProcuratorateLocker getInstance(Context context, int type) {
        if (locker == null) {
            synchronized (ProcuratorateLocker.class) {
                if (locker == null) {
                    locker = new ProcuratorateLocker(context, type);
                }
            }
        }
        return locker;
    ...
    private ProcuratorateLocker(Context context, int type) {
        lockerControl = new LockerControl(ConfigureTools.getCustomerConfigBean(context, Constant.CustomerTYPE.QIAOMU).getCom(),
                Constant.LOCKER_BAUDRATE);
        ...
        switch (handle) {
            ...
            case Constant.STATE_QUERY_SINGLE_LOCK:
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    ...
                    querySingleIntent.putExtra("isopened", Procuratorate.getDoubleOpenRes(type, HandleFactory.getCurrentBoxId(), data, context));
                    context.sendBroadcast(querySingleIntent);
                    commonDataObservable.setData(new CommonDataBean(Procuratorate.getDoubleOpenRes(type, HandleFactory.getCurrentBoxId(), data, context)));
            ...
            case Constant.STATE_OPEN_SINGLE_LOCK:
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    ...
                    openSingleIntent.putExtra("isopened", Procuratorate.getDoubleOpenRes(type, HandleFactory.getCurrentBoxId(), data, context));
            ...
            case Constant.STATE_QUERY_OPENED_LOCK:
                ...
                if (checkResultTYPE == Constant.CheckResultTYPE.RESULT_OK) {
                    boxDataObservable.addHasOpened(Procuratorate.getDoubleOpenRes(type, HandleFactory.getCurrentBoxId(), data, context));
                    boxDataObservable.setIndex(boxDataObservable.getIndex() + 1);
                }
                break;
        ...
        dataObserver.setiUpdataChipVersion(new DataObserver.IUpdataChipVersion() {
            ...
            public void updataChipVersion(ChipVersionObservable chipVersionObservable) {
                if (chipVersionObservable.getIndex() < boardAddr.length) {
                    ...
                    lockerControl.addData(CMDFactory.produceCMD(type).getChipVersion(boardAddr[chipVersionObservable.getIndex()]), false,
                            Constant.FLAG_GET_CHIP_VERSION);
                } else {
        ...
        dataObserver.setiUpdataBox(new DataObserver.IUpdataBox() {
            ...
            public void updataBox(BoxDataObservable boxDataObservable) {
                if (boxDataObservable.getIndex() < batchboxid.length) {
                    ...
                    lockerControl.addData(CMDFactory.produceCMD(type).openLock(batchboxid[boxDataObservable.getIndex()]), false,
                            Constant.STATE_OPEN_MUTI_LOCK);
                } else {
        ...
        dataObserver.setiUpdataBox(new DataObserver.IUpdataBox() {
            ...
            public void updataBox(BoxDataObservable boxDataObservable) {
                if (boxDataObservable.getIndex() < batchboxid.length) {
                    boxDataObservable.addbatchBoxId(batchboxid[boxDataObservable.getIndex()]);
                    HandleFactory.setCurrentBoxId(batchboxid[boxDataObservable.getIndex()]);
                    lockerControl.addData(CMDFactory.produceCMD(type).queryLock(batchboxid[boxDataObservable.getIndex()]), false,
                            Constant.STATE_QUERY_MUTI_LOCK);
                } else {
        ...
        dataObserver.setiUpdataBox(new DataObserver.IUpdataBox() {
            ...
            public void updataBox(BoxDataObservable boxDataObservable) {
                if (boxDataObservable.getIndex() < batchboxid.length) {
                    ...
                    HandleFactory.setCurrentBoxId(batchboxid[boxDataObservable.getIndex()]);
                    lockerControl.addData(CMDFactory.produceCMD(type).queryLock(batchboxid[boxDataObservable.getIndex()]), false,
                            Constant.STATE_QUERY_OPENED_LOCK);
                } else {
        ...
        dataObserver.setiWriteAssetCode(new DataObserver.IWriteAssetCode() {
            ...
            public void writeAssetCode(WriteAssetCodeObservable writeAssetCodeObservable) {
                if (writeAssetCodeObservable.getIndex() < list.size()) {
                    lockerControl.addData(CMDFactory.produceCMD(type).writeAssetCode(list.get(writeAssetCodeObservable.getIndex())), false,
                            Constant.FLAG_WRITE_ASSET_CODE);
                } else {
        ...
        dataObserver.setiUpdataAssetCode(new DataObserver.IUpdataAssetCode() {
            ...
            public void updataAssetCode(AssetCodeObservable assetCodeObservable) {
                ...
                if (assetCodeObservable.getIndex() != -1) {
                    lockerControl.addData(CMDFactory.produceCMD(type).readCode(assetCodeObservable.getIndex()), false,
                            Constant.STATE_HITESTER_READ_ASSETCODE);
                } else {
